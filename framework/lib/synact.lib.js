...
`;
    ...
    function PWAInstallBannerRoot({
      ...
      return div(
        { className: cx("synact-install-banner", className) },
        div(
          {},
        ...
        div(
          { className: "synact-install-actions" },
          Button({
            variant: "primary",
            onClick: onInstall,
            disabled: busy,
    ...
    function BottomNav({ items = [], activeId = null, onChange, className = "" }) {
      ...
      const safeItems = asArray(items).filter(Boolean);
      return nav(
        { className: cx("synact-bottom-nav", className), role: "navigation", "aria-label": "Bottom Navigation" },
        ...safeItems.map((item) => {
          var _a, _b;
          const itemId = (_b = (_a = item.id) != null ? _a : item.value) != null ? _b : null;
    ...
    function MobileAppShell({ title, subtitle, topActions, children, bottomNav, className = "" }) {
      ...
      const { div, h1, p } = ensureRuntime(["div", "h1", "p"]);
      return div(
        { className: cx("synact-mobile-shell", className) },
        div(
          { className: "synact-mobile-topbar" },
          div(
            {},
