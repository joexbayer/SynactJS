...
.synact-mobile-topbar {
  position: sticky;
  top: 0;
  z-index: 20;
  background: color-mix(in srgb, var(--synact-surface) 92%, transparent);
...
.synact-mobile-bottom-wrap {
  position: sticky;
  bottom: 0;
  z-index: 30;
  background: color-mix(in srgb, var(--synact-surface) 94%, transparent);
  backdrop-filter: blur(8px);
  border-top: 1px solid var(--synact-border);
}
.synact-bottom-nav {
  display: grid;
...
.synact-bottom-nav-btn-active {
  color: var(--synact-primary);
  background: color-mix(in srgb, var(--synact-primary) 14%, transparent);
  border-color: color-mix(in srgb, var(--synact-primary) 26%, transparent);
}
.synact-bottom-nav-icon {
  font-size: 16px;
  line-height: 1;
}
.synact-bottom-nav-badge {
  position: absolute;
  top: 4px;
  right: 14px;
  min-width: 16px;
...
.synact-offline-banner-danger {
  background: #991b1b;
}
.synact-install-banner {
  border: 1px solid var(--synact-border);
  border-radius: 12px;
  background: var(--synact-surface);
  padding: 12px;
...
.synact-install-title {
  margin: 0;
  font-size: 14px;
  font-weight: 700;
  color: var(--synact-text);
}
.synact-install-description {
  margin: 4px 0 0;
  color: var(--synact-text-muted);
  font-size: 12px;
}
.synact-install-actions {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}
@media (min-width: 768px) {
  .synact-mobile-shell {
    max-width: 520px;
    margin: 0 auto;
    border-left: 1px solid var(--synact-border);
    border-right: 1px solid var(--synact-border);
  }
}
`;
    ...
    function safeStorageSet(storageKey, value) {
        try {
            ...
            // Ignore blocked storage writes for non-critical UI state.
        }
    }

    function getPWAApi() {
        if (!window.SynactJS || !window.SynactJS.pwa) {
            return null;
        }

        return window.SynactJS.pwa;
    }
        ...
        const variantClass = variant === "primary"
            ...
            : variant === "ghost"
                ...
                : variant === "danger"
                    ? "synact-btn-danger"
                    : "";
        return button({ type, className: cx("synact-btn", variantClass, className), ...props }, ...normalizeChildren(children));
    }
    ...
    function Input({ label: labelText, className = "", ...props }) {
        ...
        return div({ className: "synact-input-row" },
    ...
    function SelectField({ label: labelText, options = [], className = "", ...props }) {
        ...
        const element = select({ className: cx("synact-select", className), ...props },
            ...options.map((item) => {
    ...
    function Switch({ checked = false, onChange, label: labelText, disabled = false }) {
        ...
        return div({ className: "synact-row" },
    ...
    function Progress({ value = 0, max = 100, label }) {
        ...
        return div({ className: "synact-stack" },
            label ? div({ className: "synact-row" },
                span({ className: "synact-muted" }, safeString(label)),
                span({ className: "synact-muted" }, `${pct}%`)
            ) : null,
            div({ className: "synact-progress-track" },
    ...
    function Alert({ tone = "info", title, children }) {
        ...
        return div({ className: cx("synact-alert", `synact-alert-${tone}`) },
    ...
    function KeyValueList({ entries = [] }) {
        ensureStyles();
        const { div } = ensureRuntime(["div"]);
        return div({ className: "synact-kv" },
    ...
    function EmptyState({ title = "No data", description = "There is nothing to show yet.", action }) {
        ...
        return div({ className: "synact-empty" },
    ...
    function TabsRoot({ items = [], defaultValue = null, onChange, className = "" }) {
        ...
        const initial = defaultValue != null ? defaultValue : safeItems[0]?.id;
        const [active, setActive] = useState(initial);

        const activeItem = safeItems.find((item) => item.id === active) || safeItems[0] || null;

        return div({ className: cx("synact-tabs", className) },
            div({ className: "synact-tabs-list", role: "tablist" },
            ...
            div({ className: "synact-card" },
                activeItem
                    ? (typeof activeItem.content === "function" ? activeItem.content(activeItem) : activeItem.content)
                    : ""
            )
        );
    }
    ...
    function AccordionRoot({ items = [], multiple = false, defaultOpen = [] }) {
        ...
        return div({ className: "synact-accordion" },
            ...safeItems.map((item) => {
                const open = openIds.includes(item.id);
                return div({ className: "synact-accordion-item" },
                    button({
                        type: "button",
                    ...
                    open ? div({ className: "synact-accordion-panel" },
    ...
    function Modal({ open = false, title = "", children, actions, onClose }) {
        ...
        return div({
            ...
            div({ className: "synact-modal", role: "dialog", "aria-modal": "true" },
                div({ className: "synact-modal-head" },
    ...
    function ShareButtonRoot({
        ...
        async function handleShare() {
            try {
                ...
                const fallback = fallbackCopyText || data?.url || data?.text || "";
                if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
    ...
    function ThemeToggleRoot({
        ...
        function resolveInitialTheme() {
            const stored = safeStorageGet(keyName);
        ...
        function applyTheme(theme) {
            ...
            root.classList.toggle("synact-theme-dark", theme === "dark");
            if (body) {
                body.classList.toggle("synact-theme-dark", theme === "dark");
            }
            root.setAttribute("data-theme", theme);
            safeStorageSet(keyName, theme);
        }
    ...
    function OfflineBanner(props = {}) {
        ensureStyles();
        const { h } = ensureRuntime(["h"]);
        return h(OfflineBannerRoot, props);
    }

    function OfflineBannerRoot({
        offlineText = "You are offline. Changes stay local.",
        ...
        ensureStyles();
        const { useOnlineStatus, div } = ensureRuntime(["useOnlineStatus", "div"]);
        const online = useOnlineStatus();

        if (online && hideWhenOnline) {
            return null;
        }

        const bannerText = online ? onlineText : offlineText;
        return div({
            className: cx(
                "synact-offline-banner",
                tone === "danger" ? "synact-offline-banner-danger" : "",
                className
            )
        }, safeString(bannerText));
    }

    function PWAInstallBanner(props = {}) {
        ensureStyles();
        const { h } = ensureRuntime(["h"]);
        return h(PWAInstallBannerRoot, props);
    }

    function PWAInstallBannerRoot({
        ...
        dismissible = true,
        dismissKey = "synact-install-banner-dismissed",
        hideIfDismissed = true,
        ...
        ensureStyles();
        const { useState, useEffect, div, p } = ensureRuntime(["useState", "useEffect", "div", "p"]);
        const pwa = getPWAApi();
        const dismissedByStorage = safeStorageGet(dismissKey) === "1";
        const [dismissed, setDismissed] = useState(dismissedByStorage);
        const [canPrompt, setCanPrompt] = useState(false);
        const [installed, setInstalled] = useState(false);
        const [busy, setBusy] = useState(false);

        function refreshInstallState() {
            if (!pwa || typeof pwa.getInstallState !== "function") {
                setCanPrompt(false);
                setInstalled(false);
                return;
            }
            ...
            try {
                ...
                setCanPrompt(Boolean(state.canPrompt));
        ...
        useEffect(() => {
            ...
            if (!pwa || typeof pwa.on !== "function") {
                return undefined;
            }

            const unsubscribeStatus = pwa.on("statusChange", () => refreshInstallState());
            const unsubscribePrompt = pwa.on("installPromptAvailable", () => refreshInstallState());
            const unsubscribeInstalled = pwa.on("appInstalled", () => refreshInstallState());

            return () => {
                if (typeof unsubscribeStatus === "function") unsubscribeStatus();
                if (typeof unsubscribePrompt === "function") unsubscribePrompt();
                if (typeof unsubscribeInstalled === "function") unsubscribeInstalled();
            };
        }, [pwa]);
        ...
        if (hideIfInstalled && installed) {
            return null;
        }

        if (hideIfDismissed && dismissed) {
            return null;
        }
        ...
        function onDismissNow() {
            setDismissed(true);
            safeStorageSet(dismissKey, "1");
            if (typeof onDismiss === "function") {
                onDismiss();
            }
        }

        return div({ className: cx("synact-install-banner", className) },
            div({},
                p({ className: "synact-install-title" }, safeString(title)),
                p({ className: "synact-install-description" }, safeString(description))
            ),
            div({ className: "synact-install-actions" },
                Button({
                    variant: "primary",
                    onClick: onInstall,
                    disabled: busy,
                    children: busy ? installingLabel : installLabel
                }),
                dismissible ? Button({
                    variant: "ghost",
                    onClick: onDismissNow,
    ...
    function BottomNav({ items = [], activeId = null, onChange, className = "" }) {
        ensureStyles();
        const { nav, button, a, span } = ensureRuntime(["nav", "button", "a", "span"]);
        const safeItems = asArray(items).filter(Boolean);

        return nav({ className: cx("synact-bottom-nav", className), role: "navigation", "aria-label": "Bottom Navigation" },
            ...safeItems.map((item) => {
                const itemId = item.id ?? item.value ?? null;
                const active = item.active === true || (itemId != null && itemId === activeId);
                const iconValue = item.icon == null ? null : span({ className: "synact-bottom-nav-icon" }, safeString(item.icon));
                const labelValue = span({ className: "synact-bottom-nav-label" }, safeString(item.label, safeString(itemId, "Item")));
                const badgeValue = item.badge == null ? null : span({ className: "synact-bottom-nav-badge" }, safeString(item.badge));
                const content = [iconValue, labelValue, badgeValue].filter(Boolean);
                const classes = cx("synact-bottom-nav-btn", active ? "synact-bottom-nav-btn-active" : "");
    ...
    function MobileAppShell({ title, subtitle, topActions, children, bottomNav, className = "" }) {
        ensureStyles();
        const { div, h1, p } = ensureRuntime(["div", "h1", "p"]);

        return div({ className: cx("synact-mobile-shell", className) },
            div({ className: "synact-mobile-topbar" },
                div({},
                    title ? h1({ className: "synact-mobile-title" }, safeString(title)) : null,
                    subtitle ? p({ className: "synact-mobile-subtitle" }, safeString(subtitle)) : null
                ),
                topActions ? div({ className: "synact-toolbar" }, ...normalizeChildren(topActions)) : null
            ),
            div({ className: "synact-mobile-main" }, ...normalizeChildren(children)),
            bottomNav ? div({ className: "synact-mobile-bottom-wrap" }, ...normalizeChildren(bottomNav)) : null
        );
    }
    ...
    function FileDropzoneRoot({
        ...
        return labelEl({
            ...
            div({ className: "synact-stack", style: "align-items:center;" },
            ...
            showList && names.length > 0
                ? ul({ className: "synact-file-list" }, ...names.map((name) => li({}, safeString(name))))
                : null
        );
    }
